	...
	<div id="welc">
          ...
          else
	  {
		...
		";
              ...
              {
		$stmt = $dbh->query("select id from account where username = '$username'");
                ...
                if ($passed)
                {
		  $stmt = $dbh->query("select * from account_access where id = '$id[0]'");
		  $olevel = $stmt->fetch(PDO::FETCH_BOTH);
		  if(empty($olevel))
		  {
		    $sql = "INSERT INTO `account_access` (`id`,`gmlevel`, `RealmID`) VALUES ('$id[0]','$gmlevel','-1');";
		  }
		  else
		  {
                    $sql = "UPDATE account_access SET gmlevel='$gmlevel' WHERE id = '$id[0]';";
		  }
                  $dbh->exec($sql);
                  $dbh = null;
